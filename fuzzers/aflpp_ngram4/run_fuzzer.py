...
Uses AFL_NGRAM_SIZE=4 to improve path distinction by tracking
sequences of edges instead of single edges.
...
def run_aflpp(conf, target, seeds, results, timeout_s, core, mem_limit, qemu_mode, file_mode):
    """
    Runs AFL++ with NGRAM4 coverage inside Docker.
    """
    ...
    # Inject AFL_NGRAM_SIZE=4 into environment
    ...
    cname = f"aflpp_ngram4_{mode.lower()}_{role_name}_{int(time.time()) % 10000}"

    print(f"[+] NGRAM4 mode: {mode}, role: {role_flag} {role_name}")
    print(f"[+] CMD: {cmd}")
    ...
    print(f"[+] AFL++ NGRAM4 finished. Results saved in {results}")
