...
Uses RedQueen (-l 2AT), which leverages input-to-state correspondence
to solve comparisons inside the target program.

QEMU mode additionally enables cmplog (-c 0).
"""
...
def run_aflpp(conf, target, seeds, results, timeout_s, core, mem_limit, qemu_mode, file_mode):
    """
    Runs AFL++ with RedQueen strategy inside Docker.
    """
    ...
    cname = f"aflpp_redqueen_{mode.lower()}_{role_name}_{int(time.time()) % 10000}"

    print(f"[+] REDQUEEN mode: {mode}, role={role_flag} {role_name}")
    print(f"[+] CMD: {cmd}")
...
"""
    ...
    print(f"[+] REDQUEEN finished. Results in {results}")
