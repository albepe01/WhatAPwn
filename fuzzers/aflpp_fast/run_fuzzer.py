...
Uses the AFLFast power schedule (-p fast), which prioritizes
rarely explored paths to improve coverage in complex programs.

Features:
    - Automatic dependencies installation
    - Network disabled before fuzzing
    - AFL++ executed as PID 1
...
def decide_role(results_dir: str):
    ...
    for d in existing:
        try:
            idx = int(d[len("fuzzer_s"):])
            max_idx = max(max_idx, idx)
        except:
            pass

    name = f"fuzzer_s{max_idx + 1}"
...
def run_aflpp(conf, target, seeds, results, timeout_s, core, mem_limit, qemu_mode, file_mode):
    """
    Runs AFL++ with AFLFast scheduling inside Docker.
    """
    ...
    variant = conf.get(variant_key, [])[0]

    base_args = (variant.get("args") or "").strip()
    ...
    cname = f"aflpp_fast_{mode_str.lower()}_{role_name}_{int(time.time()) % 10000}"

    print(f"[+] FAST mode: {mode_str}, CMD: {cmd}")
    ...
    print(f"[+] AFL++ FAST finished. Results saved in {results}")
...
def main():
    ...
    target, seeds, results, timeout_s, core_str, mem = sys.argv[1:7]
