#!/usr/bin/env python3
"""
AFL++ MOPT + REDQUEEN Runner

Combines:
    - MOpt mutation scheduling (-L 0)
    - RedQueen input-to-state mutation (-l 2AT)
    - Optional cmplog in QEMU mode (-c 0)

This setup improves both mutation efficiency and semantic awareness.
"""
...
def stream_logs(container, prefix, stop_event):
    """
    Streams container logs while filtering noisy AFL++ output.
    """
    try:
        for ln in container.logs(stream=True, follow=True):
            ...
            if (
                "Uh-oh" in decoded or
                "CPU cores on your system" in decoded or
                "setaffinity failed" in decoded
            ):
                continue
...
def decide_role(results_dir):
    """
    Determines MASTER (-M) or SECONDARY (-S) role via lock file.
    """
...
def run_aflpp(conf, target, seeds, results, timeout_s, core, mem_limit, qemu_mode, file_mode):
    """
    Runs AFL++ with combined MOpt + RedQueen strategy.
    """
    ...
    # Select combined strategy variant
    ...
    if dict_info:
        host, fname = dict_info
        volumes[os.path.dirname(host)] = {"bind": "/workspace/dictionary", "mode": "ro"}
        dict_arg = f"-x /workspace/dictionary/{fname}"
        print(f"[+] Dictionary: {host}")
    ...
    cname = f"aflpp_redqueen_mopt_{mode.lower()}_{role_name}_{int(time.time()) % 10000}"

    print(f"[+] REDQUEEN+MOPT mode={mode} role={role_flag} {role_name}")
    print(f"[+] CMD: {cmd}")
...
echo "[*] Starting AFL++ REDQUEEN+MOPT..."
exec {cmd}
"""
    ...
    print(f"[+] REDQUEEN+MOPT finished. Results in {results}")
