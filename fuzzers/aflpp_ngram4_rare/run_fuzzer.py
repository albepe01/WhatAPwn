#!/usr/bin/env python3
"""
AFL++ NGRAM4 + RARE Runner

Combines:
    - NGRAM coverage (AFL_NGRAM_SIZE=4)
    - Rare path prioritization (-p rare)

Focuses execution on rarely hit paths while maintaining
high path distinction via n-gram tracking.
"""
...
def run_aflpp(conf, target, seeds, results, timeout_s, core, mem_limit, qemu_mode, file_mode):
    """
    Runs AFL++ using NGRAM4 + RARE scheduling inside Docker.
    """
    ...
    cname = f"aflpp_ngram4_rare_{mode.lower()}_{role_name}_{int(time.time()) % 10000}"
...
"""
    ...
    print(f"[+] AFL++ NGRAM4+RARE finished. Results: {results}")
